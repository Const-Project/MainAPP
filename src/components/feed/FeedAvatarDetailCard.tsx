...
export default function FeedAvatarDetailCard({ postId }: Props) {
  ...
  const handleSendComment = async (commentText: string) => {
    ...
    try {
      await mutateAsync({ content: trimmedComment, targetId: postId, targetType: "AVATAR_POST" });
      setContent("");
    } catch (commentError) {
      console.error("[FeedAvatarDetailCard] Failed to post comment:", commentError);
    }
  };
  ...
  const handleDeleteComment = async (commentId: number) => {
    try {
      await deleteCommentMutation.mutateAsync({ commentId, targetType: "AVATAR_POST", targetId: postId });
  ...
  if (isHidden) {
    return (
      <View style={styles.statusCard}>
        <Text style={styles.hiddenTitle}>신고한 게시물입니다.</Text>
